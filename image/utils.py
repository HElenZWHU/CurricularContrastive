...
train_transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    # mean and std of CIFAR-10 train dataset
    transforms.Normalize([0.49139968,0.48215841,0.44653091] ,[0.24703223,0.24348513,0.26158784])
    ])

test_transform = transforms.Compose([
    transforms.ToTensor(),
    # mean and std of CIFAR-10 test dataset
...
def get_dataset(dataset_name, root='../data', pair=True):
    if pair:
        if dataset_name == 'svhn':
            train_data = SVHNPair(root=root, split='train', transform=train_transform)
            memory_data = SVHNPair(root=root, split='train', transform=test_transform)
            test_data = SVHNPair(root=root, split='test', transform=test_transform)
        ...
        if dataset_name in ['cifar10', 'cifar10_true_label']:
            ...
            train_data = SVHN(root=root, split='train', transform=train_transform)
            memory_data = SVHN(root=root, split='train', transform=test_transform)
            test_data = SVHN(root=root, split='test', transform=test_transform)
        else:
            raise Exception('Invalid dataset name')
